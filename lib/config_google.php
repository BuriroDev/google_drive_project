<?php

class Config
{

    const GOOGLE_WEB_CLIENT_ID = '653081781599-vv88bskel1osssvntcjltnhgjvfd5kmj.apps.googleusercontent.com';

    const GOOGLE_WEB_CLIENT_SECRET = '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';

    const GOOGLE_ACCESS_SCOPE = 'https://www.googleapis.com/auth/drive';

    const AUTHORIZED_REDIRECT_URI = 'http://localhost/Intern_Tasks/google_drive_project/lib/callback_google.php';

    const GOOGLE_OAUTH2_TOKEN_URI = 'https://oauth2.googleapis.com/token';

    const GOOGLE_DRIVE_FILE_UPLOAD_URI = 'https://www.googleapis.com/upload/drive/v3/files';

    const GOOGLE_DRIVE_FILE_META_URI = 'https://www.googleapis.com/drive/v3/files/';
}
